...
| \[M-1\] | Lack of accountability of `wTAO` tokens can lead to users not being able to unstake their tokens |  Medium | Fixed |
| \[M-2\] | Users will not be able to request unstakes if the ether transfer to the `withdrawalManager` fails.  |  Medium | Acknowledged |
| \[L-1\] | In `requestUnstake()`, the `unstakingFee` is used as if it was measured in wTAO units, but it is expressed as a percentage (base 1000), making it trivial to bypass the requirement `yTAOAmtAcknowledged> unstakingFee` |  Low |  Fixed |
| \[L-2\] | Pulling ether from the `yTAO` contract will revert if the recipient is not an EOA |  Low | Fixed |
| \[L-3\] | The `wrap()` function will revert when users attempt to wrap exactly `minStakingAmt` |  Low | Fixed |
| \[L-4\] | The `wrap()` function requires that the caller's `wTAO` balance is higher than the amount to stake (`wtaoAmount`) but ignores the fees in the requirement |  Low | Fixed |
| \[L-5\] | The function `approveMultipleUnstakes()` lacks protection against repeated unstake requests |  Low | Acknowledged |
| \[L-6\] | The parameter `minStakingAmount` cannot be ever set to 0, due to a wrong requirement in `setMinStakingAmount()`  |  Low | Fixed |
| \[L-7\] | The `maxDepositPerRequest` is not taken into account when calculating `maxTaoForWrap()` which will create reverts if users try to wrap the max amount |  Low | Fixed |
| \[L-8\] | In future contract updates, maxSupply needs to be checked against initial supply  |  Low | Acknowledged |
| \[L-9\] | The usage of Ownable & Access control is redundant as they are both serving the same purpose. |  Low | Acknowledged |
| \[L-10\] | Use SafeERC20 library for ERC20 transfers |  Low | Partly fixed |
...
| \[G-2\] | Checking for the `wrappedToken` in every `UnstakeRequest` is expensive and unnecessary if the `wTAO` token is not expected to change |  Gas | Fixed |
...
| \[G-4\] | Unnecessary repeated check every time the exchange rate is updated |  Gas | Fixed |
...
| \[G-7\] | Save gas by storing bytes instead of strings for `nativeWalletReceiver` |  Gas | Fixed |
| \[I-1\] | The `checkPaused` modifier is used in `approveMultipleUnstakes()` |  Informational | Fixed |
| \[I-2\] | The nonReentrant modifier should be placed first for security |  Informational | Fixed |
| \[I-3\] | Wrong argument name in events  |  Informational | Fixed |
